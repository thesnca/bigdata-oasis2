...
from oasis.api.khbase.results.khbase_cluster import KhbaseCluster
from oasis.api.khbase.results.khbase_cluster_summary import KhbaseClusterSummary
...
async def describe_cluster_from_db(cluster_id, account_id=None):
    ...
    kes_cluster = KhbaseCluster(cluster).__dict__
...
async def list_clusters_from_db(filters, offset, limit, *,
    ...
    query = query.filter(ClusterModel.cluster_type == 'KHBASE')
    ...
    for cluster in clusters:
        summary_cluster = KhbaseClusterSummary(cluster).__dict__
        instance_id_list = summary_cluster.get('InstanceIdList', {})
