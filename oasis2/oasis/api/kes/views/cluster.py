...
from oasis.api.kes.methods import describe_cluster_from_db
from oasis.api.kes.methods import list_clusters_from_db
...
class ClusterView(BaseView):
    ...
    async def describe_cluster(self, *args, **kwargs):
        ...
        web_service = proxy_services.get('WebService', {}) or {}
        cluster.setdefault('ProxyServices', list(web_service.values()))

        service_status = await gringotts_monitor_client.list_cluster_status(
            cluster_id, cluster.get('ClusterType'), token=self.user_token)
        status = service_status.get('status', []) if type(service_status) is dict else []
        cluster_status = None
        for stat in status:
            if stat.get('cluster_id', '') == cluster_id:
                cluster_status = stat.get('status', None)
        cluster.setdefault('ServiceStatus', cluster_status)
    ...
    async def launch_cluster(self, *args, **kwargs):
        ...
        if distribution not in DISTRIBUTION_SCHEMAS.get('KES', []):
            raise Exception(f'KES did not support distribution {distribution}')

        # TODO cache
        product_details = await price_client.get_product_details(self.account_id, PRODUCT_GROUP_ID_MAP['KES'], '1')
        ...
        job_2 = JobModel(name='replace_resources_tags', status=JobModel.STATUS.Init, cluster_id=cluster_id,
                         parent_job=tags_job_id)
        await job_2.save()
        job_2_id = job_2.id

        task_replace_resources_tags_bind_eip = TaskModel(name='TaskReplaceResourcesTags',
                                                         args={
                                                             'exec_mode': TagResource.EXEC.NEW,
                                                             'tags': tags
                                                         })
        task_2_graph = {
            task_replace_resources_tags_bind_eip: []
        }
        await save_task_graph(job_2_id, task_2_graph)
    ...
    async def delete_cluster(self, *args, **kwargs):
        ...
        if cluster.charge_type == 'Monthly' and not is_expire:
            raise Exception(f'Cluster charge type is Monthly, could not be deleted.')
        ...
        task_graph = {
            task_send_feishu_init: [],
            task_milestone_1: [task_gringotts_delete_cluster, task_delete_inner_lb],
            task_remove_instance_monitor: [task_remove_cluster_monitor],
            task_remove_cluster_monitor: [task_gringotts_delete_cluster, task_delete_inner_lb],
        }
        ...
        if not is_expire:  # 如果是主动退订，需要调用统一退订的订单接口退订所有的服务实例，然后调用各业务线的api退订实例
            ...
            task_graph.setdefault(task_delete_inner_lb, [task_update_instance_status])
            task_graph.setdefault(task_update_instance_status, [task_milestone_2])

        delete_ks3_user_plugins = [ep.id for ep in cluster.es_plugins
                                   if ep.plugin_type == EsPluginModel.SOURCE.USER_DEFINE_PLUGIN and
                                   ep.upload_type == EsPluginModel.TYPE.DEFAULT_UPLOAD_TYPE]
        task_delete_ks3_user_plugins = TaskModel(
            name='TaskDeleteKs3UserPlugins',
            args={
                'plugins': delete_ks3_user_plugins
            }
        )

        # task_graph.setdefault(task_milestone_2, [task_send_feishu_done])
        # task_graph.setdefault(task_send_feishu_done, [])
        task_graph.setdefault(task_milestone_2, [task_delete_ks3_user_plugins])
        task_graph.setdefault(task_delete_ks3_user_plugins, [task_send_feishu_done])
        task_graph.setdefault(task_send_feishu_done, [])
        ...
        if need_disassociate_eip:
            ...
            # Delete SLB
            # 释放集群时，需要在符合需求且避免”知乎问题“的情况下，删除公私SLB
        ...
        # region 释放集群默认标签
        job_2 = JobModel(name='delete_cluster_default_tag', status=JobModel.STATUS.Init, cluster_id=cluster_id,
                         parent_job=parent_job_id)
        await job_2.save()
        job_2_id = job_2.id
        parent_job_id = job_2.id

        await save_task_graph(
            job_2_id, {
                TaskModel(name='TaskDeleteClusterDefaultTags'): []
            }
        )
        # endregion
    ...
    async def freeze_cluster(self, *args, **kwargs):
        ...
        task_graph = {
            task_send_feishu_init: [],
            task_milestone_1: [task_stop_instance],
    ...
    async def unfreeze_cluster(self, *args, **kwargs):
        ...
        task_graph = {
            ...
            task_start_instance: [task_milestone_2],
