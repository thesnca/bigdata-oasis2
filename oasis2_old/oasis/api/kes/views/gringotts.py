...
class GringottsView(BaseView):
    """
        ...
        routes.append(('/GetSnapshotStatus', GringottsView))
    """
    ...
    async def enable_xpack(self, *args, **kwargs):
        cluster_id = kwargs.get('cluster_id', None)
        account_id = self.account_id
        if not cluster_id:
            raise Exception(f'Please specify cluster id, got {cluster_id}')
        ...
        job = JobModel(name='enable_xpack', status=JobModel.STATUS.Init, cluster_id=cluster_id)
        job_id = gen_uuid4()
        job.id = job_id
    ...
    async def disable_xpack(self, *args, **kwargs):
        cluster_id = kwargs.get('cluster_id', None)
        account_id = self.account_id
        if not cluster_id:
            raise Exception(f'Please specify cluster id, got {cluster_id}')
        ...
        job = JobModel(name='disable_xpack', status=JobModel.STATUS.Init, cluster_id=cluster_id)
        job_id = gen_uuid4()
        job.id = job_id
        ...
        # gringotts禁用Xpack
        task_service_control = TaskModel(
            name='TaskGringottsDisableXpack',
            args=kwargs,
        )
    ...
    async def snapshot_on(self, *args, **kwargs):
        cluster_id = kwargs.get('cluster_id', None)
        account_id = self.account_id
        if not cluster_id:
            raise Exception(f'Please specify cluster id, got {cluster_id}')
        ...
        if cluster.status != ClusterModel.STATUS.ACTIVE:
            raise Exception(f'Cannot snapshot on, cluster status: {cluster.status}')

        job = JobModel(name='snapshot_on', status=JobModel.STATUS.Init, cluster_id=cluster_id)
        job_id = gen_uuid4()
        job.id = job_id
        ...
        # gringotts开启备份配置：新增透传参数
        task_snapshot_on = TaskModel(
            name='TaskGringottsSnapshotOn',
            args=kwargs,
        )
        ...
        task_graph = {
            task_send_feishu_init: [],
            task_snapshot_on: [task_send_feishu_done],
            task_send_feishu_done: [],
        }
